	...
	case WM_COMMAND:
	{
		...
		case MainApp::ControlsID::OKBUTTON_ID:
		{
			try
			{
				...
				{
					MessageBox(this->hWndMain, L"Введите адрес", L"Информация", MB_ICONINFORMATION | MB_OK);
					break;
				}
				...
				{
					MessageBox(this->hWndMain, L"Введите путь", L"Информация", MB_ICONINFORMATION | MB_OK);
					break;
				}
				...
				//this->lexems.assign(lexemObj.returnChoosenLexems().begin(), lexemObj.returnChoosenLexems().end());
...
void MainApp::createControls()
{
	...
	if (this->hWndEditPath = CreateWindowEx(
		WS_EX_CLIENTEDGE,
		L"EDIT",
		L"C:\\Users\\user\\source\\repos\\Parser\\MainApp\\test.txt",
		WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL,
